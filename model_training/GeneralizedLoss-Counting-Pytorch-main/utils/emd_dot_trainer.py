...
# from models.vgg_dilated_backend import vgg19 as vgg_dilated
from models.vgg19csr import vgg19 # <-- vggcsr2
# from models.Res101_SFCN import Res101_SFCN
# from models.resnet50 import Res50
# from models.resnet101 import Res101
...
def grid(H, W, stride):
    coodx = torch.arange(0, W, step=stride) + stride / 2  # divide by 2 as pixels should go in between grids ie:  0.5  1.5
...
class EMDTrainer(Trainer):
    def setup(self):
        ...
        self.dataloaders = {x: DataLoader(self.datasets[x],
                                          ...
                                          num_workers=args.num_workers*1, # using single gpu only
                                          pin_memory=(True if x == 'train' else False), drop_last=True)
                            for x in ['train', 'val']}

        self.model = vgg19() #Res50()  # Res101_SFCN() # vgg19()
        # self.model = SCAR()
        # self.model = vgg16()
        # self.model = vgg_dilated()
        # self.model = Res101()


        self.model.to(self.device)
        self.optimizer = optim.Adam(self.model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        # self.scheduler = StepLR(self.optimizer, step_size=1, gamma=0.995) ## SFCN
        ...
        if args.resume:
            ...
            if suf == 'tar':
                ...
                # self.scheduler.load_state_dict(checkpoint['scheduler']) ## SFCN
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, args.max_epoch):
            ...
            # if epoch > 9: ## SFCN
            #     self.scheduler.step()  ## SFCN

    def train_eopch(self, epoch=0, stage='train'):
        ...
        for step, (inputs, points, targets, st_sizes) in enumerate(self.dataloaders['train']):
            ...
            targets = [t.to(self.device) for t in targets]  # targets = target head size from knn=3 avg head distances.
        ...
        _loss = epoch_loss.get_avg()
        _mse = np.sqrt(epoch_mse.get_avg())
        _mae = epoch_mae.get_avg()
        self.writer.add_scalar(stage+'/loss', _loss, self.epoch)
        self.writer.add_scalar(stage+'/mae', _mae, self.epoch)
        self.writer.add_scalar(stage+'/mse', _mse, self.epoch)
        logging.info('Epoch {} Train, Loss: {:.2f}, MSE: {:.2f} MAE: {:.2f}, Cost {:.1f} sec'
                    .format(self.epoch, _loss, _mse, _mae, time.time()-epoch_start))
    ...
    def val_epoch(self, stage='val'):
        ...
        # for step, (inputs, points, name) in enumerate(dataloader):
            inputs = inputs.to(self.device)
            # inputs are images with different sizes
            assert inputs.size(0) == 1, 'the batch size should equal to 1 in validation mode'
            with torch.set_grad_enabled(False):
                # print(step, name, inputs.shape)
                outputs = self.model(inputs)
