...
class Crowd(data.Dataset):
    def __init__(self, root_path, crop_size,
        ...
        if is_gray:
            ...
            self.trans = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize([0.446139603853, 0.409515678883, 0.395083993673], [0.288205742836, 0.278144598007, 0.283502370119]) #nwpu
    ...
    def __getitem__(self, item):
        ...
        name = os.path.basename(img_path).split('.')[0] # for checking training , to be removed

        if self.method == 'train':
            return self.train_transform_with_crop(img, keypoints, name)
    ...
    def train_transform_with_crop(self, img, keypoints, name):
        ...
        if len(keypoints) > 0:
            ...
            if random.random() > 0.5:
                img = F.hflip(img)
        return self.trans(img), torch.from_numpy(keypoints.copy()).float(), \
               torch.from_numpy(target.copy()).float(), st_size, name
